  ...
  target.innerHTML = `
    <div class="site-specimen">
      ...
      <section class="specimen-services" id="services" aria-labelledby="services-title">
        ...
        <div class="service-list" id="work">${content.services.map(([name, copy], index) => `<article><span class="service-index">0${index + 1}</span><h3>${name}</h3><p>${copy}</p><a href="#contact">Explore service <span aria-hidden="true">↗</span></a></article>`).join('')}</div>
      </section>
      ...
      <section class="specimen-contact" id="contact" aria-labelledby="contact-title"><div><p class="specimen-kicker">Start a project</p><h2 id="contact-title">Tell us what you're thinking.</h2></div><a class="specimen-primary" href="#contact">${content.primaryAction} <span aria-hidden="true">↗</span></a></section>
