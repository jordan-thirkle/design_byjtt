...
for (const route of routes) {
  test(`canonical public shell: ${route}`, async ({ page }) => {
    ...
    await expect(nav.locator('a')).toHaveCount(expectedLabels.length + 1); // brand + 6 nav + CTA
...
test('canonical public shell has mobile information architecture', async ({ page }) => {
